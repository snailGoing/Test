...
public class AlertDialogFragment extends DialogFragment implements
    ...
    public static class EditTextDialogFragment extends AlertDialogFragment {
                    ...
                    if (source != null && source.length() > 0 && (oldSize + newSize) > maxLength) {
                        ...
                        for (i = 0 ; i < newText.length() && j > 0; i++) {
                            ...
                            if (getStrBytesNum(addStr + tmp) > addLen) {
                                Log.d(TAG, "not add char any more because of beyond max length if added this char.");
                                flag = true;
                                break;
                            }
                            addStr += tmp;
                        ...
                        if (addStr != null && addStr.length() > 0) {
                            ...
                            if (getStrBytesNum(lastChar) + getStrBytesNum(tmp) != getStrBytesNum(lastChar + tmp)) {
                                Log.d(TAG, "do not seperate one whole code into two part");
                                if (addStr.length() == 1) {
                                    addStr = "";
                                } else {
                        ...
                        Log.w(TAG, "input out of range,hasVibrator: addLen " + addLen + " addStr " +  addStr + " isVibrator " + hasVibrator);
                        return addStr;
                    }
        ...
        public void onClick(DialogInterface dialog, int which) {
            ...
            if (mEditText != null && inputMgr != null && inputMgr.isActive()) {
                ...
                if (token != null) {
                    ...
                    Log.e(TAG, "EditTextDialogFragment onClick(): error cannot get window token");
                }
            }
        ...
        private int getStrBytesNum(String inStr) {
            ...
            Log.d(TAG, "getStrBytesNum, inStr size =" + strSize + ",inStr =" + inStr);
            return strSize;
        }
    ...
    public static class ChoiceDialogFragment extends AlertDialogFragment {
        ...
        @Override
        public Dialog onCreateDialog(Bundle savedInstanceState) {
            Log.d(CHOICE_DIALOG_TAG, "Show alertSortDialog");
            Builder builder = createAlertDialogBuilder(savedInstanceState);
