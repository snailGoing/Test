...
class DetailInfoTask extends BaseAsyncTask {
    ...
    protected Integer doInBackground(Void... params) {
        ...
        if (mDetailfileInfo.isFolder()) {
            if (!MountRootManagerRf.getInstance().isRootPath(
                    mDetailfileInfo.getPath())) {
    ...
    public int getContentSize(File root) {
        ...
        if (root.isDirectory()) {
            ...
            if (files == null) {
                Log.d(TAG, "getContentSize, listFiles is null, return");
                return ret;
            }
            for (File file : files) {
                if (isCancelled()) {
                ...
                ret = getContentSize(file);
                if (ret < 0) {
                    Log.i(TAG, "getContentSize ,ret = " + ret);
                    return ret;
                }
            }
        }
