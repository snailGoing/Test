...
public class FileInfoManager {
    ...
    private final ArrayList<FileInfo> mShowFilesInfoList = new ArrayList<FileInfo>();
    ...
    public void updateFileInfoList(String currentPath, int sortType) {
        Log.d(TAG, "updateFileInfoList,currentPath = " + currentPath + "sortType = "
                + sortType);
    ...
    public FileInfo updateOneFileInfoList(String path, int sortType) {
        Log.d(TAG, "updateOneFileInfoList,path = " + path + "    sortType = " + sortType+"  show old size="+ mShowFilesInfoList.size());
        FileInfo fileInfo = null;
        mLastAccessPath = path;
        mModifiedTime = (new File(mLastAccessPath)).lastModified();
        Log.d(TAG, "updateOneFileInfoList,add size= " +mAddFilesInfoList.size() );
        if (mAddFilesInfoList.size() > 0) {

            fileInfo = mAddFilesInfoList.get(0);
            Log.d(TAG, "updateOneFileInfoList,add fileInfo= " +fileInfo.getPath() );
            if (fileInfo.getFileParentPath().equals(mLastAccessPath)) {
                mShowFilesInfoList.add(fileInfo);
            }
        }
        Log.d(TAG, "updateOneFileInfoList, new show size= " + mShowFilesInfoList.size() );
        mShowFilesInfoList.removeAll(mRemoveFilesInfoList);
        mPasteFilesInfoList.removeAll(mRemoveFilesInfoList);
        mAddFilesInfoList.clear();
        mRemoveFilesInfoList.clear();
    ...
    public void loadFileInfoList(String path, int sortType) {
        Log.d(TAG, "loadFileInfoList,path = " + path + ",sortType = " + sortType);
        mShowFilesInfoList.clear();
        mLastAccessPath = path;
        mModifiedTime = (new File(mLastAccessPath)).lastModified();
        if (MountRootManagerRf.getInstance().isRootPath(path)) {
            mAddFilesInfoList.clear();
            List<FileInfo> fileInfoList = new ArrayList<FileInfo>();
            List<FileInfo> mountFileList = MountRootManagerRf.getInstance().getMountPointFileInfo();
            Log.d(TAG, "mountFileList size = " + mountFileList.size());
            if (mountFileList != null) {
                fileInfoList.addAll(mountFileList);
            }
            Log.d(TAG, "fileInfoList size = " + fileInfoList.size());
            addItemList(fileInfoList);
        }
        Log.d(TAG, " mAddFilesInfoList size :" + mAddFilesInfoList.size());
        final FileInfo[] addFilesInfos = new FileInfo[mAddFilesInfoList.size()];
        mAddFilesInfoList.toArray(addFilesInfos);
        for (FileInfo fileInfo : addFilesInfos) {
            Log.d(TAG, "fileinfo is :" + fileInfo.toString());
            if (mLastAccessPath.equals(fileInfo.getFileParentPath())
                    || MountRootManagerRf.getInstance().isMountPoint(
                fileInfo.getPath())) {
                Log.d(TAG, "mShowFilesInfoLis add fileinfo " + fileInfo.getName());
                mShowFilesInfoList.add(fileInfo);
            }
        }
        mAddFilesInfoList.clear();
    ...
    public void loadFileInfoList(String path, int sortType, FileInfo selectedFileInfo) {
        ...
        for (FileInfo fileInfo : mAddFilesInfoList) {
            ...
            if (fileInfo == null) {
                Log.w(TAG, "loadFileInfoList,file info is null!");
                continue;
            }
            if (mLastAccessPath.equals(fileInfo.getFileParentPath())
                    || MountRootManagerRf.getInstance().isMountPoint(fileInfo.getPath())) {
                mShowFilesInfoList.add(fileInfo);
                if (selectedFileInfo != null && fileInfo.getName().equals(selectedFileInfo.getName())) {
    ...
    public NavigationRecord getPrevNavigation() {
        while (!mNavigationList.isEmpty()) {
            ...
            if (!TextUtils.isEmpty(path)) {
                if (new File(path).exists() || MountRootManagerRf.getInstance().isRootPath(path)) {
                    return navRecord;
                }
            }
        }
     ...
     */
    public ArrayList<FileInfo> getShowFileList() {
        Log.d(TAG, "getShowFileList: size= "+mShowFilesInfoList.size());
     ...
     */
    public void obtainSearchList() {
        Log.d(TAG, "obtainSearchList");
        mShowFilesInfoList.clear();
