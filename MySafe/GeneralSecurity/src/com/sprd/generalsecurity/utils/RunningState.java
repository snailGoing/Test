...
//    final InterestingConfigChanges mInterestingConfigChanges = new InterestingConfigChanges();
    ...
    public final Comparator<RunningState.MergedItem> mBackgroundComparator
    ...
    final Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case MSG_REFRESH_UI:
                Log.i(TAG,
                        msg.arg1 != 0 ? "OnRefreshUiListener.REFRESH_STRUCTURE"
                ...
                case MSG_UPDATE_TIME:
                    ...
                    if (mRefreshUiListener != null) {
                        Log.i(TAG, "mRefreshUiListener != null");
    ...
    public void sendUpdateMessage(){
        Message m = mHandler.obtainMessage(MSG_UPDATE_TIME);
        mHandler.sendMessageDelayed(m, TIME_UPDATE_DELAY);
    }
        ...
        public void onReceive(Context context, Intent intent) {
            synchronized (mLock) {
                if (mResumed) {
                    ...
                    Log.i(TAG,"----UserManagerBroadcastReceiver----");
...
//            context.registerReceiverAsUser(this, UserHandle.ALL, filter, null, null);
    ...
    public static class ProcessItem extends BaseItem {
        ...
        public void ensureLabel(PackageManager pm) {
            ...
            //SPRD: Bug 675507 java.lang.NullPointerException: Attempt to get length of null array
        ...
        boolean updateService(Context context, ActivityManager.RunningServiceInfo service) {
            ...
            Log.i(TAG,"updateService*1 ServiceItem.mCurSeq="+si.mCurSeq+" ;mCurSeq="+mCurSeq);
            si.mCurSeq = mCurSeq;
            si.mRunningService = service;
        ...
        boolean buildDependencyChain(Context context, PackageManager pm, int curSeq) {
            ...
            Log.i(TAG,"NP="+NP+" ; mDependentProcesses="+mDependentProcesses);
            boolean changed = false;
            for (int i=0; i<NP; i++) {
                ...
                Log.i(TAG,"buildDependencyChain*1 ProcessItem.mCurSeq="+proc.mCurSeq+" ;curSeq="+curSeq);
                proc.mCurSeq = curSeq;
                proc.ensureLabel(pm);
...
//                    mInterestingConfigChanges.applyNewConfig(mApplicationContext.getResources());
    ...
    private void addOtherUserItem(Context context, ArrayList<MergedItem> newMergedItems,
        ...
        Log.i(TAG,"addOtherUserItem first = "+first);
        if (first) {
            ...
            Log.i(TAG,"infirst*2 userItem.mCurSeq="+userItem.mCurSeq+" ;mSequence="+mSequence);
            userItem.mUser = new UserState();
            userItem.mUser.mInfo = info;
//            userItem.mUser.mIcon = MemoryUtils.getUserIcon(context, mUm, info);
//            userItem.mUser.mLabel = MemoryUtils.getUserLabel(context, info);
    ...
    private boolean update(Context context, ActivityManager am) {
        ...
        for (int i=0; i<NS; i++) {
            ...
            Log.i(TAG,"\t\t"+i+"  si="+si+" name="+si.service.getClassName());
        ...
        for (int i=0; i<NP; i++) {
            ...
            Log.i(TAG,"\t\t"+i+"  pi="+pi+" name="+pi.processName);
            mTmpAppProcesses.put(pi.pid, new AppProcessInfo(pi));
        }
        Log.i(TAG,"mTmpAppProcesses size= "+mTmpAppProcesses.size());
        ...
        for (int i=0; i<NS; i++) {
            ...
            Log.i(TAG,"**#1 ProcessItem.mCurSeq="+proc.mCurSeq+" ;mSequence="+mSequence);
            if (proc.mCurSeq != mSequence) {
                ...
                Log.i(TAG,"**#2 ProcessItem.mCurSeq="+proc.mCurSeq+" ;mSequence="+mSequence);
        ...
        for (int i=0; i<NP; i++) {
            ...
            if (isInterestingProcess(pi)) {
                ...
                Log.i(TAG,"**#3 ProcessItem.mCurSeq="+proc.mCurSeq+" ;mSequence="+mSequence);
                proc.mCurSeq = mSequence;
        ...
        for (int i=0; i<NAP; i++) {
            ...
            Log.i(TAG,"**#4 ProcessItem.mCurSeq="+proc.mCurSeq+" ;mSequence="+mSequence);
            if (proc.mCurSeq == mSequence) {
        ...
        for (int i=0; i<mServiceProcessesByName.size(); i++) {
            ...
            while (pit.hasNext()) {
                ...
                Log.i(TAG,"**#5 ProcessItem.mCurSeq="+pi.mCurSeq+" ;mSequence="+mSequence);
                ...
                while (sit.hasNext()) {
                    ...
                    Log.i(TAG,"**#6 ServiceItem.mCurSeq="+si.mCurSeq+" ;mSequence="+mSequence);
        ...
        if (changed) {
            ...
            for (int i=0; i<NU; i++) {
                ...
                Log.i(TAG,"NU*1 MergedItem.mCurSeq="+user.mCurSeq+" ;mSequence="+mSequence);
       ...
       Log.i(TAG,"**: mRunningProcesses="+mRunningProcesses+" ;size="+NRP);
        for (int i=0; i<NRP; i++) {
            ProcessItem proc = mRunningProcesses.valueAt(i);
            Log.i(TAG,i+"*: proc="+proc+" ;proc.mCurSeq="+proc.mCurSeq+" ;mSequence="+mSequence);
            if (proc.mCurSeq != mSequence) {
                // We didn't hit this process as a dependency on one
                // of our active ones, so add it up if needed.
                Log.i(TAG,
                        "\t\timportance="
                                + proc.mRunningProcessInfo.importance
                ...
                if (proc.mRunningProcessInfo.importance >=
                        ActivityManager.RunningAppProcessInfo.IMPORTANCE_BACKGROUND) {
                    Log.i(TAG, "numBackground:" + numBackgroundProcesses
                            + " ; proc=" + proc);
                    numBackgroundProcesses++;
                    mAllProcessItems.add(proc);
                } else if (proc.mRunningProcessInfo.importance <=
                        ActivityManager.RunningAppProcessInfo.IMPORTANCE_VISIBLE) {
                    Log.i(TAG, "numForeground:" + numForegroundProcesses
                            + " ; proc=" + proc);
        ...
        try {
            ...
            for (int i=0; i<pids.length; i++) {
                ...
                Log.i(TAG,"pids*1 ProcessItem.mCurSeq="+proc.mCurSeq+" ;mSequence="+mSequence);
                if (proc.mCurSeq == mSequence) {
                    ...
                    Log.i(TAG,"mBackgroundItems #1="+mBackgroundItems+" size="+mBackgroundItems.size());
                    ...
                    Log.i(TAG,"mBackgroundItems #2="+mBackgroundItems+" size="+mBackgroundItems.size());
        ...
        Log.i(TAG,"numBackgroundProcesses= "+numBackgroundProcesses+"  ; mBackgroundItems.size()="+mBackgroundItems.size());
        if (newBackgroundItems == null) {
        ...
        if (newBackgroundItems != null) {
            ...
            if (!diffUsers) {
                ...
                Log.i(TAG,"newUserBackgroundItems 2 = "+newUserBackgroundItems);
                ...
                for (int i=0; i<NU; i++) {
                    ...
                    Log.i(TAG,"NU*2 MergedItem.mCurSeq="+user.mCurSeq+" ;mSequence="+mSequence);
                    if (user.mCurSeq == mSequence) {
        ...
        synchronized (mLock) {
            ...
            Log.i(TAG, "newBackgroundItems = "+newBackgroundItems+" ; newUserBackgroundItems="+newUserBackgroundItems);
            if (newBackgroundItems != null) {
    ...
    public ArrayList<MergedItem> getCurrentMergedItems() {
        synchronized (mLock) {
            Log.i(TAG,"mMergedItems="+mMergedItems);
            return mMergedItems;
        }
    }
    ...
    public ArrayList<MergedItem> getCurrentBackgroundItems() {
        synchronized (mLock) {
            Log.i(TAG, "mUserBackgroundItems=" + mUserBackgroundItems);
