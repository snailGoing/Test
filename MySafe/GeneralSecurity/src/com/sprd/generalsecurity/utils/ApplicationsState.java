...
public class ApplicationsState {
    ...
    public final ArrayList<Session> mSessions = new ArrayList<Session>();
    final ArrayList<Session> mRebuildingSessions = new ArrayList<Session>();
    // final InterestingConfigChanges mInterestingConfigChanges = new
    // InterestingConfigChanges();
    // Map: userid => (Map: package name => AppEntry)
    final SparseArray<HashMap<String, AppEntry>> mEntriesMap = new SparseArray<HashMap<String, AppEntry>>();
    ...
    private ApplicationsState(Application app) {
        ...
        for (UserHandle user : mUm.getUserProfiles()) {
            mEntriesMap.put(user.getIdentifier(),
    ...
    void doResumeIfNeededLocked() {
        ...
        for (UserHandle user : mUm.getUserProfiles()) {
            try {
                ...
                ParceledListSlice<ApplicationInfo> list = mIpm
                        .getInstalledApplications(
                                user.isOwner() ? mOwnerRetrieveFlags
                                        : mRetrieveFlags, user.getIdentifier());
        ...
        for (int i = 0; i < mApplications.size(); i++) {
            ...
            final AppEntry entry = mEntriesMap.get(userId)
    ...
    public void requestSize(String packageName, int userId) {
        ...
        synchronized (mEntriesMap) {
            AppEntry entry = mEntriesMap.get(userId).get(packageName);
            if (entry != null) {
                Log.i(TAG, "entry before:"
                        + (entry.cacheSize + entry.externalCacheSize) + "   :"
                        + entry.cacheSize + "   " + entry.externalCacheSize);
                mPm.getPackageSizeInfoAsUser(packageName, userId,
        ...
        try {
            synchronized (mEntriesMap) {
                ...
                ApplicationInfo info = mIpm.getApplicationInfo(pkgName,
                        userId == UserHandle.USER_OWNER ? mOwnerRetrieveFlags
                                : mRetrieveFlags, userId);
                if (info == null) {
                    return;
                }
                if (!info.enabled) {
    ...
    private void addUser(int userId) {
        if (mUm.getUserProfiles().contains(new UserHandle(userId))) {
            synchronized (mEntriesMap) {
                ...
                if (mResumed) {
                    // If resumed, Manually pause, then cause a resume to
                    // repopulate the app list.
                    // This is the simplest way to reload the packages so that
                    // the new user
                    // is included. Otherwise the list will be repopulated on
                    // next resume.
                    doPauseLocked();
    ...
    public class Session {
        ...
        // comparator.
        public ArrayList<AppEntry> rebuild(AppFilter filter,
            ...
            synchronized (mRebuildSync) {
                ...
                mRebuildAsync = true;
                /* start */
                Log.i(TAG, "mRebuildResult:" + mRebuildResult);
                for (AppEntry app : mRebuildResult) {
                    Log.i(TAG, "name:" + app.label + "  " + app.cacheSize
            ...
            for (int i = 0; i < apps.size(); i++) {
                ...
                if (filter == null || filter.filterApp(entry)) {
                    synchronized (mEntriesMap) {
                        ...
                        if (DEBUG)
                            Log.i(TAG, "Using " + entry.info.packageName + ": "
                                    + entry);
    ...
    class MainHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            case MSG_PACKAGE_SIZE_CHANGED: {
                Log.i(TAG, mActiveSessions.size() + "    msg.obj\t"
                        + msg.obj);
                for (int i = 0; i < mActiveSessions.size(); i++) {
        ...
        public void handleMessage(Message msg) {
            ...
            case MSG_LOAD_ENTRIES: {
                ...
                synchronized (mEntriesMap) {
                    ...
                    for (int i = 0; i < mApplications.size() && numDone < 6; i++) {
                        ...
                        if (userId != 0 && mEntriesMap.indexOfKey(0) >= 0) {
                            // If this app is for a profile and we are on the
                            // owner, remove
                            // the owner entry if it isn't installed. This will
                            // prevent
                            // duplicates of work only apps showing up as 'not
                            // installed
                            // for this user'.
                            // Note: This depends on us traversing the users in
                            // order, which
                            // happens because of the way we generate the list
                            // in
                            // doResumeIfNeededLocked.
                            AppEntry entry = mEntriesMap.get(0).get(
                                    info.packageName);
                            if (entry != null
                ...
                if (numDone >= 6) {
                    ...
                    if (!mMainHandler
                            .hasMessages(MainHandler.MSG_LOAD_ENTRIES_COMPLETE)) {
                        mMainHandler
                                .sendEmptyMessage(MainHandler.MSG_LOAD_ENTRIES_COMPLETE);
                    }
            ...
            case MSG_LOAD_LAUNCHER: {
                ...
                for (int i = 0; i < mEntriesMap.size(); i++) {
                    ...
                    List<ResolveInfo> intents = mPm
                            .queryIntentActivitiesAsUser(launchIntent,
                                    PackageManager.GET_DISABLED_COMPONENTS,
                                    userId);
                    synchronized (mEntriesMap) {
                        if (DEBUG_LOCKING)
                            Log.v(TAG, "MSG_LOAD_LAUNCHER acquired lock");
                        HashMap<String, AppEntry> userEntries = mEntriesMap
                                .valueAt(i);
                        ...
                        if (DEBUG_LOCKING)
                            Log.v(TAG, "MSG_LOAD_LAUNCHER releasing lock");
                    }
                }

                if (!mMainHandler
                        .hasMessages(MainHandler.MSG_LAUNCHER_INFO_CHANGED)) {
                    mMainHandler
                            .sendEmptyMessage(MainHandler.MSG_LAUNCHER_INFO_CHANGED);
                }
                    ...
                    for (int i = 0; i < mAppEntries.size(); i++) {
                        ...
                        if (entry.size == SIZE_UNKNOWN || entry.sizeStale) {
                            if (entry.sizeLoadStart == 0
                                ...
                                mCurComputingSizeUserId = UserHandle
                                        .getUserId(entry.info.uid);
        ...
        final IPackageStatsObserver.Stub mStatsObserver = new IPackageStatsObserver.Stub() {
            public void onGetStatsCompleted(PackageStats stats,
                ...
                synchronized (mEntriesMap) {
                    ...
                    HashMap<String, AppEntry> userMap = mEntriesMap
                            .get(stats.userHandle);
                    if (userMap == null) {
                    ...
                    if (entry != null) {
                        ...
                        synchronized (entry) {
                            ...
                            if (entry.size != newSize
                                ...
                                Log.i(TAG, "entry sizeChanged:true");
                                entry.size = newSize;
                                entry.cacheSize = stats.cacheSize;
                                entry.codeSize = stats.codeSize;
                                entry.dataSize = stats.dataSize;
                    ...
                    if (mCurComputingSizePkg != null
                            && (mCurComputingSizePkg.equals(stats.packageName) && mCurComputingSizeUserId == stats.userHandle)) {
                        mCurComputingSizePkg = null;
                        sendEmptyMessage(MSG_LOAD_SIZES);
                    }
                    if (DEBUG_LOCKING)
                        Log.v(TAG, "onGetStatsCompleted releasing lock");
                    Log.i(TAG, "entry after:"
                            + (entry.cacheSize + entry.externalCacheSize)
                            + "   :" + entry.cacheSize + "   "
                            + entry.externalCacheSize);
                }
    ...
    private class PackageIntentReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_PACKAGE_ADDED.equals(actionStr)) {
                ...
                addUser(intent.getIntExtra(Intent.EXTRA_USER_HANDLE,
                        UserHandle.USER_NULL));
            } else if (Intent.ACTION_USER_REMOVED.equals(actionStr)) {
                removeUser(intent.getIntExtra(Intent.EXTRA_USER_HANDLE,
                        UserHandle.USER_NULL));
            }
        }
    }
    ...
    public static class AppEntry extends SizeInfo {
        ...
        // A location where extra info can be placed to be used by custom
        // filters.
        ...
        private Drawable getBadgedIcon(PackageManager pm) {
            // Do badging ourself so that it comes from the user of the app not
            // the current user.
        ...
        public String getVersion(Context context) {
            try {
                return context.getPackageManager().getPackageInfo(
                        info.packageName, 0).versionName;
    ...
    public static class CompoundFilter implements AppFilter {
        ...
        @Override
        public boolean filterApp(AppEntry info) {
            return mFirstFilter.filterApp(info)
                    && mSecondFilter.filterApp(info);
        }
    }
}