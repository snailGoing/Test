...
public class LockScreenFlowFragment extends Fragment{
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (mDualSim) {
            mRootView = inflater.inflate(R.layout.app_flow_dual, container, false);
        } else {
            if (mNetworkType != WIFI) {//not WIFI, then set network type as sim1
                mNetworkType = SIM1;
            }
            mRootView = inflater.inflate(R.layout.app_flow_single, container, false);
        }
        return mRootView;
    }
    ...
    private void initViews(){
        ...
        mWifiFlowTotalText.setText(String.format(getResources().getString(R.string.data_flow_wifi),
                Formatter.formatFileSize(mContext, mWifiUsageTotal)));
        ...
        if (mDualSim) {
            mSim1FlowTotalText.setText(String.format(getResources().getString(R.string.data_flow_data1),
                    Formatter.formatFileSize(mContext, mSim1UsageTotal)));
            ...
            mSim2FlowTotalText.setText(String.format(getResources().getString(R.string.data_flow_data2),
                    Formatter.formatFileSize(mContext, mSim2UsageTotal)));
        } else {
    ...
    private void updateUsageTotalData() {
        ...
        if (mDualSim) {
            mSim1FlowTotalText.setText(String.format(getResources().getString(R.string.data_flow_data1),
                    Formatter.formatFileSize(mContext, mSim1UsageTotal)));
            // SPRD: Bug706839 it doesn't show the value of Sim2UsageTotal when sim2 is primary
            mSim2FlowTotalText.setText(String.format(getResources().getString(R.string.data_flow_data2),
                    Formatter.formatFileSize(mContext, mSim2UsageTotal)));
        } else {
            mSim1FlowTotalText.setText(String.format(getResources().getString(R.string.data_flow_data),
    ...
    private UidDetail buildUidDetail(int uid) {
        ...
        switch (uid) {
            ...
            case TrafficStats.UID_TETHERING:
                /* SPRD: Bug 676414 Attempt to write to field UidDetail.usageSim1' on a null object reference @{ */
    ...
    class AppRankAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (convertView == null) {
                if (!mDualSim) {
                    convertView = mInflater.inflate(R.layout.flow_rank_item_single, null);
                } else {
                    convertView = mInflater.inflate(R.layout.flow_rank_item_dual, null);
            ...
            if(mDualSim) {
                holder.flow_data.setText(String.format(getResources().getString(R.string.data_flow_data1),
                        Formatter.formatFileSize(mContext, mApps.get(position).usageSim1)));
            } else {
            ...
            holder.flow_wifi.setText(String.format(getResources().getString(R.string.data_flow_wifi),
                    Formatter.formatFileSize(mContext, mApps.get(position).usageWifi)));
            holder.flow_sum.setText(String.format(getResources().getString(R.string.data_flow_sum),
                    Formatter.formatFileSize(mContext, mApps.get(position).usageSum)));
            if (mDualSim) {
                holder.flow_data2.setText(String.format(getResources().getString(R.string.data_flow_data2),
                        Formatter.formatFileSize(mContext, mApps.get(position).usageSim2)));
            }
